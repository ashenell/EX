...
public class Persons {
    ...
    public void findsPersonById() {
        int idToFind = 2;
        Optional<Person> first = persons.stream().filter(p -> p.id().equals(idToFind))
                .findFirst();
    ...
    public void removePersonById() {
        int idToFind = 2;
        persons = persons.stream().filter(p -> !p.id().equals(idToFind))
                .toList();
    ...
    public void findsPersonNames() {
        String names = persons.stream().map(p -> p.name())
